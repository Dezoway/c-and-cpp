...
DLinkedList* loadToList(DLinkedList *list);//Загрузка списка из файла
void quit(DLinkedList *list);//Выход
int main(void){
    ...
    while(1!=0){
        ...
        case 7:
            ...
            /*if(loadToList(list)){
                printf("Список изданий успешно загружен.\n");
                system("pause");
                }
            else{
...
DLinkedList* loadToList(DLinkedList* list){
    if(list != NULL){ //Очистка списка если он существует
    Node* tmp = (list)->head;
        while(tmp){
    ...
    list = (DLinkedList*) malloc(sizeof(DLinkedList)); //Выделение памяти под новый список
    list->head = NULL;
    list->tail = NULL;
    FILE *input;
    char file[SIZE];
